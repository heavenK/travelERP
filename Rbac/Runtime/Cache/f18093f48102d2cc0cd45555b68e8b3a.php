...
<!-- 主体内容  -->
<div class="content" >
<div class="title"> 模块授权  [ <a href="__URL__">返 回</a> ]</div>
<!--  功能组区域  -->
<script language="JavaScript">
<!--
function saveAccess(){
ThinkAjax.sendForm('form1','__URL__/setModule/');
}
//-->
</script>
...
<tr><th class="tCenter"><a href="__URL__/app/groupId/<?php echo ($_GET['groupId']); ?>">应用授权</a> | 模块授权 | <a href="__URL__/action/groupId/<?php echo ($_GET['groupId']); ?>">操作授权</a>
</th></tr>
...
<tr><Td class="tRight">
当前应用：<select id="" name="appId" onchange="location.href = '?appId='+this.options[this.selectedIndex].value;" ondblclick="" class="medium" ><option value="" >选择应用</option><?php  foreach($appList as $key=>$val) { if(!empty($selectAppId) && ($selectAppId == $key || in_array($key,$selectAppId))) { ?><option selected="selected" value="<?php echo $key ?>"><?php echo $val ?></option><?php }else { ?><option value="<?php echo $key ?>"><?php echo $val ?></option><?php } } ?></select>
</Td></tr>
<tr><td >
<select id="groupModuleId" name="groupModuleId[]" ondblclick="" onchange="" multiple="multiple" class="multiSelect" size="15" ><?php  foreach($moduleList as $key=>$val) { if(!empty($groupModuleList) && ($groupModuleList == $key || in_array($key,$groupModuleList))) { ?><option selected="selected" value="<?php echo $key ?>"><?php echo $val ?></option><?php }else { ?><option value="<?php echo $key ?>"><?php echo $val ?></option><?php } } ?></select>
</td>
</tr>
<tr>
