...
<!-- 主体内容  -->
<div class="content" >
<div class="title">用户组列表 [ <a href="__URL__">返 回</a> ]</div>
<!--  功能组区域  -->
<script language="JavaScript">
<!--
function saveAccess(){
ThinkAjax.sendForm('form1','__URL__/setRole/');
}
...
<table class="select" style="width:265px" ALIGN='center'><tr><td height="5" colspan="3" class="topTd" ></td></tr><tr><th class="tRight">当前用户：<select id="" name="userId" onchange="location.href = '__URL__/role/id/'+this.options[this.selectedIndex].value;" ondblclick="" class="medium" ><option value="" >选择用户</option><?php  foreach($userList as $key=>$val) { if(!empty($selectUserId) && ($selectUserId == $key || in_array($key,$selectUserId))) { ?><option selected="selected" value="<?php echo $key ?>"><?php echo $val ?></option><?php }else { ?><option value="<?php echo $key ?>"><?php echo $val ?></option><?php } } ?></select>
</th></tr>
<tr><td >
<select id="sourceS" name="userGroupId[]" ondblclick="" onchange="" multiple="multiple" class="multiSelect" size="15" ><?php  foreach($groupList as $key=>$val) { if(!empty($userGroupList) && ($userGroupList == $key || in_array($key,$userGroupList))) { ?><option selected="selected" value="<?php echo $key ?>"><?php echo $val ?></option><?php }else { ?><option value="<?php echo $key ?>"><?php echo $val ?></option><?php } } ?></select>
</td>
</tr>
<tr>
