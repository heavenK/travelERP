...
function setGroupActions($groupId,$actionIdList)
{
	...
	function getGroupUserList($groupId)
	{
		...
		$rs = $this->db->query('select b.user_id,b.user_name from '.$table.' as a ,'.$this->tablePrefix.'glkehu as b where a.user_id=b.user_id and  a.role_id='.$groupId.' ');
		return $rs;
	}
	...
	function setGroupUsers($groupId,$userIdList)
	{
		...
		$where = 'a.id ='.$groupId.' AND b.user_id in('.$userIdList.')';
		$rs = $this->execute('INSERT INTO '.$this->tablePrefix.'role_user (role_id,user_id) SELECT a.id, b.user_id FROM '.$this->tablePrefix.'role a, '.$this->tablePrefix.'glkehu b WHERE '.$where);
		if($result===false) {
			return false;
		}else {
			return true;
