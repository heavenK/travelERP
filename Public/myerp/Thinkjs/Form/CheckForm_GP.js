...
*/
	...
	function CheckForm(oForm,target)
	{
		...
		{
			//是否需要验证
			if(els.item(i).getAttribute('check'))
			{
				//取得验证的正则字符串
				var sReg = Filter(trim(els.item(i).getAttribute('check')));
				//取得表单的值,用通用取值函数
				...
				if(!reg.test(sVal))
				{
					...
					if (target==undefined)
					{
						alert(els.item(i).getAttribute('warning'));
					}else {
						
//						document.getElementById(target).style.display='block';
						document.getElementById(target).innerHTML	=	'<div style="color:red">'+els.item(i).getAttribute('warning')+'</div>';
						...
						this.intval = window.setTimeout(function (){
							...
							document.getElementById(target).style.display='none';document.getElementById(target).innerHTML='';
							},3000);
					}
