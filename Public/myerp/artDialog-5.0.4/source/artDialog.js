...
var jQuery = window.art = function (selector, context) {
        return new jQuery.fn.constructor(selector, context);
    },
    quickExpr = /^(?:[^<]*(<[\w\W]+>)[^>]*jQuery|#([\w\-]+)jQuery)/,
    rclass = /[\n\t]/g;
...
jQuery.fn = jQuery.prototype = {
    ...
    */
    bind: function (type, callback) {
        jQuery.event.add(this[0], type, callback);
        return this;
    },
    ...
    */
    unbind: function(type, callback) {
        jQuery.event.remove(this[0], type, callback);
        return this;
    }
    
};

jQuery.fn.constructor.prototype = jQuery.fn;
...
function getElementsByClassName (className, node, tag) {
    ...
    var i = 0,
        ...
        elsLen = els.length,
        pattern = new RegExp("(^|\\s)" + className + "(\\s|jQuery)");
...
jQuery.uuid = 0;
jQuery.cache = {};
jQuery.expando = '@cache' + (+ new Date);

// 标记元素唯一身份
function uuid (elem) {
    var expando = jQuery.expando,
        id = elem === window ? 0 : elem[expando];
    if (id === undefined) elem[expando] = id = ++ jQuery.uuid;
    return id;
};
...
* 事件机制
* @namespace
* @requires	[jQuery.data, jQuery.removeData]
*/
jQuery.event = {
    ...
    */
    add: function (elem, type, callback) {
        var cache, listeners,
            that = jQuery.event,
            data = jQuery.data(elem, '@events') || jQuery.data(elem, '@events', {});
...
/**
* 清理元素集的事件与缓存
* @requires	[jQuery.removeData, jQuery.event]
* @param		{HTMLCollection}	元素集
*/
jQuery.cleanData = function (elems) {
    var i = 0, elem,
        len = elems.length,
        removeEvent = jQuery.event.remove,
...
// 获取css
jQuery.css = 'defaultView' in document && 'getComputedStyle' in document.defaultView ?
    function (elem, name) {
...
/**
* 获取滚动条位置 - [不支持写入]
* jQuery.fn.scrollLeft, jQuery.fn.scrollTop
* @example		获取文档垂直滚动条：jQuery(document).scrollTop()
...
* @example 
获取文档宽度：jQuery(document).width()
获取可视范围：jQuery(window).width()
* @return	{Number}
*/
...
var _singleton,
    _count = 0,
    _root = jQuery(document.getElementsByTagName('html')[0]),
...
artDialog.fn = artDialog.prototype = {
    ...
    */
    content: function (message) {
        ...
        var prev, next, parent, display,
            that = this,
            jQuerycontent = this.dom.content,
            content = jQuerycontent[0];
    ...
    */
    title: function (content) {
        ...
        var dom = this.dom,
            outer = dom.outer,
            jQuerytitle = dom.title,
            className = 'd-state-noTitle';
        
        if (content === false) {
            jQuerytitle.hide().html('');
            outer.addClass(className);
        } else {
            jQuerytitle.show().html(content);
    ...
    position: function () {
        ...
        var dom = this.dom,
            wrap = dom.wrap[0],
            jQuerywindow = dom.window,
            jQuerydocument = dom.document,
            fixed = this.config.fixed,
            dl = fixed ? 0 : jQuerydocument.scrollLeft(),
            dt = fixed ? 0 : jQuerydocument.scrollTop(),
    ...
    */
    follow: function (elem) {
        ...
        var fixed = config.fixed,
            ...
            winWidth = jQuerywindow.width(),
            winHeight = jQuerywindow.height(),
            docLeft =  jQuerydocument.scrollLeft(),
            docTop = jQuerydocument.scrollTop(),
            offset = jQueryelem.offset(),
    ...
    */
    button: function () {
        ...
        var dom = this.dom,
            jQuerybuttons = dom.buttons,
            elem = jQuerybuttons[0],
        ...
        for (; i < ags.length; i ++) {
            ...
            if (val.focus) {
                ...
                this._focus = jQuery(button).addClass(strongButton);
                this.focus();
            };
        ...
        jQuerybuttons[0].style.display = ags.length ? '' : 'none';
    ...
    close: function () {
        ...
        // 使用单例模式
            ...
            jQuerywrap[0].className = jQuerywrap[0].style.cssText = '';
    ...
    */
    ...
    lock: function () {
        ...
        jQuerydiv.bind('dblclick', function () {
            that._click('cancel');
        });
        
        document.body.appendChild(div);
        
        this._lockMask = jQuerydiv;
        this._isLock = true;
    ...
    _innerHTML: function (data) {
        ...
        .replace(/{([^}]+)}/g, function (jQuery0, jQuery1) {
            var value = data[jQuery1];
...
// 快捷方式绑定触发元素
jQuery.fn.dialog = jQuery.fn.artDialog = function () {
...
if (jQuery.fn.live) {
    jQuery('body').live('focus', focusin);
} else if (document.addEventListener) {
    document.addEventListener('focus', focusin, true);
} else {
    jQuery(document).bind('focusin', focusin);
}
...
this.artDialog = jQuery.dialog = jQuery.artDialog = artDialog;
