 ...
 function insertItem(divname)
 {
	...
	htmlcontent += "</td>";
	htmlcontent += "<td scope=\"row\" align=\"left\" valign=\"top\">";
	htmlcontent += SITE_INDEX+"<input type=\"text\" id=\"url_t"+i+"\" style=\"width:200px;\" >";
	htmlcontent += "</td>";
	htmlcontent += "<td scope=\"row\" align=\"left\" valign=\"top\">";
	htmlcontent += "</td>";
	htmlcontent += "<td scope=\"row\" align=\"left\" valign=\"top\">";
 ...
 function save(id,divname,mark)
 {
	...
	jQuery.ajax({
		type:	"POST",
		url:	SITE_INDEX+"SetSystem/dopostSystemHas/tableName/directory",
		data:	"title="+title+"&url="+url+it,
 ...
 function om_save(data,status,info,type,id,divname)
 {
	if(status == 1){
		...
		htmlcontent += "</td>";
		htmlcontent += "<td scope=\"row\" align=\"left\" valign=\"top\">";
		htmlcontent += SITE_INDEX+"<input type=\"text\" id=\"url"+data['systemID']+"\" style=\"width:200px;\" value=\""+data['url']+"\">";
		htmlcontent += "</td>";
		htmlcontent += "<td scope=\"row\" align=\"left\" valign=\"top\">";
		htmlcontent += "</td>";
		htmlcontent += "<td scope=\"row\" align=\"left\" valign=\"top\">";
		...
		htmlcontent += "<input class=\"button\" type=\"button\" value=\"子目录\" onClick=\"subdirectory("+data['systemID']+");\" /></td>";
		htmlcontent += "</tr>";
 ...
 function deleteSystemItem(id,divname,type)
 {
	...
	jQuery.ajax({
		type:	"POST",
		url:	SITE_INDEX+"SetSystem/deleteSystemItem/tableName/directory",
		data:	"systemID="+id,
		success:function(msg){
...
function subdirectory (systemID)
{
	window.location.href=SITE_INDEX+"SetSystem/directory/parentID/"+systemID; 
}
