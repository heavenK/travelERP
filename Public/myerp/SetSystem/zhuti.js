 ...
 function save(id,divname,mark)
 {
	...
	var description = jQuery("#description"+mark+id).val();
	jQuery.ajax({
		type:	"POST",
		url:	SITE_INDEX+"SetSystem/dopostSystemHas/tableName/datadictionary",
		data:	"type=主题&title="+title+"&description="+description+it,
 ...
 function om_save(data,status,info,type,id,divname)
 {
	if(status == 1){
		...
		htmlcontent += "</td>";
		htmlcontent += "<td scope=\"row\" align=\"left\" valign=\"top\"><input type=\"text\" id=\"description"+data['systemID']+"\" style=\"width:200px\" ></td>";
		htmlcontent += "<td scope=\"row\" align=\"left\" valign=\"top\">";
