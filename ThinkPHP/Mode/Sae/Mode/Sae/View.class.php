...
// +----------------------------------------------------------------------
// $Id: View.class.php 2442 2011-12-18 08:29:00Z luofei614@gmail.com $
 ...
 * @subpackage  Core
 * @author liu21st <liu21st@gmail.com>
 * @version  $Id: View.class.php 2442 2011-12-18 08:29:00Z luofei614@gmail.com $
 +------------------------------------------------------------------------------
 */
class View extends Think{
    ...
    protected function checkCache($tmplTemplateFile, $mtime) {//sae下判断模版缓存时间，增加mtime参数
        ...
        if (filemtime($tmplTemplateFile) > $mtime) {
            // 模板文件如果有更新则缓存需要更新
            return false;
        } elseif (C('TMPL_CACHE_TIME') != 0 && time() > $mtime + C('TMPL_CACHE_TIME')) {
            // 缓存是否在有效期
            return false;
        }
        //缓存有效
