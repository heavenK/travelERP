    ...
    protected function init() {
        ...
        if ($user_name==$unadmin && $user_id==$uidadmin) {
			...
			if($roleuser)
			{
				...
				$lvxingshe = M('Gllvxingshe')->where("`lvxingsheID`='$roleuser[lvxingsheID]'")->find();
				$this->company = $lvxingshe;
			}
			//读取部门表
			$mydepartment = M('glbasedata')->where("`id`='$roleuser[department]'")->find();
			if($mydepartment)
			{
				$this->my_department = $mydepartment;
			}
			//读取职位
			$myrole = M('glbasedata')->where("`id`='$roleuser[role]'")->find();
			if($myrole)
			{
				$this->my_role = $myrole;
			}
			//读取权限表
			$adminuser = M('Gladminuser')->where("`user_id`='$user_id'")->find();
			if($adminuser)
			{
