...
{//类定义开始
    ...
    private $name =  '';
    public $my     =  '';
    public static $login_user ='';//model api
    public static $site_info ='';//model
    public static $plugin_av ='';//model available plugins
    public $site   =  null;
   ...
   /**
    ...
    function sysinfo() {
        ...
        if ($user_name && $user_id) {
            self::$login_user=$this->my = $uModel->where("user_id='$user_id' AND user_name='$user_name'")->find();
        } else {
            $this->my='';
        }
        //网站信息
        self::$site_info=$this->site=F('site');
        if (!$this->site) {
            $site=array();
            ...
            self::$site_info=$this->site=$site;
            F('site',$site);
        }
        //伪静态设置
        if (intval($this->site['openrewrite'])==0) {
            define('SITE_URL',ET_URL."/home.php");
            C('URL_MODEL',1);
        } else if (intval($this->site['openrewrite'])==2) {
            define('SITE_URL',ET_URL."/home.php?s=");
            C('URL_MODEL',3);
        } else {
        ...
        //插件获取
        self::$plugin_av=$this->plugincache=F('plugincache');
        if (!$this->plugin) {
            $allplugins=array();
            $pageplugins='';
            ...
            self::$plugin_av=$this->plugincache=$plus;
            F('plugincache',$plus);
        }
        //vipcss
        $vipgroup=F('vipgroup');
        if ($vipgroup) {
            foreach($vipgroup as $val){
                $vipcss.=".vip{$val[id]} {background:url('$val[iconurl]') no-repeat right;padding-right:16px;}";
            }
            $this->assign('vipcss',$vipcss);
        } else {
            $vipgroup = D('Vipgroup')->select();
            F('vipgroup',$vipgroup);
        }
    }
    function init() {
        $uModel=M('Users');
        //设置缓存类型
        C('DATA_CACHE_TYPE',$this->site['cachetype']);
        C('MEMCACHE_HOST',$this->site['memhost']);
        //向导
        if ($this->my['user_id'] && $this->site['regmailauth']==1 && $this->my['regmailauth']==0 && strtolower(ACTION_NAME)!='regmailauth' && strtolower(ACTION_NAME)!='sendmail' && strtolower(ACTION_NAME)!='mailactivity' && strtolower(ACTION_NAME)!='logout' && strtolower(ACTION_NAME)!='changemail') {
            header('location:'.SITE_URL.'/Index/regmailauth');
            exit;
        }
        if ($this->my['user_id'] && $this->my['userguide']==0 && strtolower(MODULE_NAME)!='guide' && (($this->site['regmailauth']==1 && $this->my['regmailauth']==1) || $this->site['regmailauth']==0) && strtolower(ACTION_NAME)!='logout' && strtolower(MODULE_NAME)!='api') {
            header('location:'.SITE_URL.'/Guide');
            exit;
        }
        $apprun=@include(RUNTIME_PATH.'~'.APP_CACHE_NAME.'.php');
        if (is_array($apprun) && $apprun['url_model']!=C('URL_MODEL')) {
            App::build(C('URL_MODEL'));
        }
        //在线时间
        if ($this->my['user_id'] && time()-$this->my['last_login']>600) { //最后登陆时间
            $uModel->where("user_id='".$this->my['user_id']."'")->setField('last_login',time());
        }
        //网站统计
        if ($this->my['user_id'] && $this->my['login_date']!=date('Ymd')) {
            $uModel->where("user_id='".$this->my['user_id']."'")->setField('login_date',date('Ymd'));
            $this->addtongji('login');
        }
        //初始化插件函数
        $plugin= new pluginManager('view');
        if (!$_GET['rand'] && !$_POST) {
            $hook=$plugin->do_view();
        }
        //安全转码
        if ((C('default_lang')=='zh-cn' || C('default_lang')=='zh-tw') && (!$_COOKIE['etlang'] || $_COOKIE['etlang']=='zh-tw' || $_COOKIE['etlang']=='zh-cn')) {
            $_GET=getSafeCode($_GET,'gb2312');
        }

        $setok=Cookie::get('setok');
        Cookie::set('setok',-1);
        if ($setok && $setok!=-1) {
            $this->assign('setok',L($setok));
        }
        $this->assign('my',$this->my);
        $this->assign('site',$this->site);
        $this->assign('plugin',$plugin);
        $this->assign('plugincache',$this->plugincache);
        $this->assign('hook',$hook);
        //关闭网站
        if ($this->site['webclose']==1 && $this->my['isadmin']==0 && ACTION_NAME!='login' && ACTION_NAME!='dologin') {
            $this->assign('type','webclose');
            $this->display('Error/index');
            exit;
        }
    }

    function addtongji($type) {
        $tj=D('Tongji');

        $data=$tj->where("dateline='".date('Ymd')."' AND type='$type'")->find();
        if ($data) {
            $tj->where("dateline='".date('Ymd')."' AND type='$type'")->setInc('nums');
        } else {
            $insert['nums']=1;
            $insert['dateline']=date('Ymd');
            $insert['type']=$type;
            $tj->add($insert);
        }
    }
