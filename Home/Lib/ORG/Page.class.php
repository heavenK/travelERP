...
class Page extends Think {
    ...
    public function show($url){
        ...
        $url=str_replace(array('%3F','%3D','%26','%2F'),array('?','=','&','/'),$url);
        $url=SITE_URL.'/'.$url;

        //参数
        $csarray=array('hq','dt','group');
        foreach ($csarray as $val) {
            if ($_GET[$val]) {
                $cs[]=$val.'='.$_GET[$val];
            }
        }
        ...
        if ($upRow>0){
            $upPage="<a href='".$url."$upRow{$css}'>".$this->config['prev']."</a>";
        }else{
            $upPage="<span class=\"disabled\">".$this->config['prev']."</span>";
        }

        if ($downRow <= $this->totalPages){
            $downPage="<a href='".$url."$downRow{$css}'>".$this->config['next']."</a>";
        }else{
        ...
        if($nowCoolPage == 1){
            ...
            $prePage = "<a href='".$url."$preRow{$css}' >上".$this->rollPage."页</a>";
            $theFirst = "<a href='".$url."1{$css}' class='first' >".$this->config['first']."</a>";
        }
        if($nowCoolPage == $this->coolPages){
            ...
            $nextPage = "[<a href='".$url."$nextRow{$css}' >下".$this->rollPage."页</a>]";
            $theEnd = "<a href='".$url."$theEndRow{$css}' class='last' >".$this->config['last']."</a>";
        }
        // 1 2 3 4 5
        $linkPage = "";
        for($i=1;$i<=$this->rollPage;$i++){
            ...
            if($page!=$this->nowPage){
                if($page<=$this->totalPages){
                    $linkPage .= "<a href='".$url."$page{$css}'>".$page."</a>";
                }else{
                    break;
                }
            }else{
